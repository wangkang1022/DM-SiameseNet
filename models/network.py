...
class FourLayer_64F(nn.Module):
	def __init__(self, norm_layer=nn.BatchNorm2d, num_classes=5, neighbor_k=3):
		...
		self.classifier = DM_SiameseNet_Metric(neighbor_k=neighbor_k)
	...
	def forward(self, input1, input2):
		...
		for i in range(len(input2)):
			support_set_sam = self.features(input2[i])#torch.Size([5, 64, 21, 21])
			# support_set_sim = support_set_sam.mean(0)

			S_sim.append(support_set_sam)
			B, C, h, w = support_set_sam.size()
			support_set_sam = support_set_sam.permute (1, 0, 2, 3)#support_set_sam.size(),torch.Size([64, 2205]) 21*21*5=2205
			support_set_sam = support_set_sam.contiguous().view(C, -1)

			S_con.append(support_set_sam)

		x = self.classifier(q, S_con, S_sim ) # get Batch*num_classes

		return x


# ========================== Define an DM_SiameseNet ==========================#
class DM_SiameseNet_Metric(nn.Module):
	def __init__(self, num_classes=5, neighbor_k=3):
		super(DM_SiameseNet_Metric, self).__init__()
		self.neighbor_k = neighbor_k

		self.fully_connect1 = torch.nn.Linear(64*21*21, 1)
		# self.fully_connect2 = torch.nn.Linear(512, 1)
		self.Norm_layer = nn.BatchNorm1d(num_classes*2, affine=True)
		self.FC_layer = nn.Conv1d(1, 1, kernel_size=2, stride=1, dilation=5, bias=False)



	def ILM_similarity(self, input1, input2):
		...
		for i in range(len(input1)):
			...
			for j in range(len(input2)):
				...
				temp_support_sim = []
				for k in range(len(input2[j])):
					temp_support = torch.flatten(input2[j][k], 0)
					s_dis = torch.abs(temp_query - temp_support)
					s_dis = self.fully_connect1(s_dis)
					s_dis = torch.tanh(s_dis)
					temp_support_sim.append(s_dis)

				temp_dis = sum(temp_support_sim) / len(temp_support_sim)
				inner_sim[0, j] = temp_dis
	...
	def I2C_similarity(self, input1, input2):
		B, C, h, w = input1.size()
		Similarity_list = []
		...
		for i in range(B):
			...
			for j in range(len(input2)):
				...
				innerproduct_matrix = query_sam@support_set_sam#torch.Size([441, 2205])

				# choose the top-k nearest neighbors
				topk_value, topk_index = torch.topk(innerproduct_matrix, self.neighbor_k, 1)#topk_value :torch.Size([441, 3])
				inner_sim[0, j] = torch.sum(topk_value)/len(query_sam)/(self.neighbor_k)
	...
	def similarity(self, input1_batch, input2_batch, input3_batch):
		'''
			input1_batch: (75*4) * 441 * 64     75 query images
			input2_batch: (25*4) * 441 * 64     5-way 5-shot
		'''

		inner_sim = self.I2C_similarity(input1_batch, input2_batch)

		x1 = torch.flatten(input1_batch, 1)
		sia_sim = self.ILM_similarity(x1, input3_batch)

		kl_sim_soft2 = torch.cat((sia_sim, inner_sim), 1)  # .unsqueeze(1)
	...
	def forward(self, x1, x2, x):
		...
		Similarity = self.similarity(x1, x2, x)
